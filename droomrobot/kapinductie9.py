...
class Kapinductie9(DroomrobotScript):
    ...
    def _introduction(self):
        ...
        # Fixed scaffold: position + breathing (~30-45s).
        ...
        #              f'Laten we alvast gaan oefenen om samen een mooie droomreis te maken naar {self.user_model['droomplek_lidwoord']} {self.user_model['droomplek']}, zodat het je zometeen gaat helpen bij de slaapdokter.')
        ...
        # Block for Prompt D result (should be done by now)
        self.add_move(self._store_intervention_result)
    ...
    def _intervention(self):
        ...
        self.phase_moves_build = InteractionChoice('Kapinductie9', InteractionChoiceCondition.PHASE)
    ...
    #def _build_interaction_choice_droomplek(self) -> InteractionChoice:
        ...
        interaction_choice.add_move('vakantie', self.droomrobot.say,
                                    'Het was er heerlijk warm in de zon.')
    ...
    def _intervention_preparation(self, phase_moves: InteractionChoice) -> InteractionChoice:
        # Set slow speaking rate + animations
        ...
        intervention_prep_choice.add_move('bos', self.droomrobot.say,
                                          'Stel je maar voor dat je weer in het bos bent, op die fijne plek.')
        intervention_prep_choice.add_move('bos', self.droomrobot.say,
                                          'Kijk maar weer naar alle mooie kleuren die er zijn en merk hoe fijn je je voelt op deze plek.')
        ...
        intervention_prep_choice.add_move('ruimte', self.droomrobot.say,
                                          'Stel je maar voor dat je weer in de ruimte bent, boven de aarde, omgeven door de sterren.')
        intervention_prep_choice.add_move('ruimte', self.droomrobot.say,
                                          'Kijk maar naar de sterren die glinsteren, voel maar hoe rustig het is in deze uitgestrekte ruimte.')
        intervention_prep_choice.add_move('ruimte', self.droomrobot.say,
                                          'Luister naar het zachte geluid van je ademhaling en de stilte om je heen.')
        intervention_prep_choice.add_move('ruimte', self.droomrobot.say,
                                          'Je mag naar het ruimteschip toe zweven en er lekker in gaan zitten.')
        intervention_prep_choice.add_move('ruimte', self.droomrobot.say,
    ...
    def _play_intervention_sentences(self):
        ...
        sentences = payload.get('intervention_preparation') or self._get_fallback_intervention_imagery(9)
