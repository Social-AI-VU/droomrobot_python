...
class Sonde9(DroomrobotScript):

    def __init__(self, *args, **kwargs):
        super(Sonde9, self).__init__(*args, **kwargs, interaction_context=InteractionContext.BLOEDAFNAME)
    ...
    def _introduction(self):
        ...
        self.add_move(self.droomrobot.say, 'Jij kunt namelijk heel makkelijk jezelf helpen in het ziekenhuis door je lichaam en hoofd te ontspannen.')
        ...
        self.add_move(self.droomrobot.say, 'En wat nu zo handig is aan dit truukje is dat het je ook kan helpen met het inbrengen van de sonde.')
        self.add_move(self.droomrobot.say, 'Door ergens anders aan te denken, zoals iets dat jij leuk vind, richt je je aandacht daarop in plaats van de sonde.')
        self.add_move(self.droomrobot.say, 'Dat maakt het vaker minder ongemakkelijk, en kan het fijner laten voelen.')
        ...
        self.add_move(self.droomrobot.say,  'Voel hoe je buik zachtjes op en neer beweegt bij iedere ademhaling.')
        ...
        self.add_move(self.droomrobot.say,  'Als je genoeg geoefend hebt, mag je je ogen weer lekker open doen.')
        self.add_move(self.droomrobot.animate, AnimationType.EXPRESSION, "emo_007", run_async=True)  ## Smile
        self.add_move(self.droomrobot.say,  lambda: f'Ik vind {self.user_model['kleur']} een hele mooie kleur, die heb je goed gekozen.')

        self.add_move(self.droomrobot.say,
                      lambda: f'Als je straks aan de beurt bent ga ik je vragen om in gedachten terug te gaan naar {self.user_model['droomplek_locatie']}.')
    ...
    def _intervention(self):
        ...
        self.phase_moves_build = InteractionChoice('Sonde9', InteractionChoiceCondition.PHASE)
    ...
    def _intervention_preparation(self, phase_moves: InteractionChoice) -> InteractionChoice:
        ...
        # Raceauto
        intervention_prep_choice.add_move('raceauto', self.droomrobot.say,
                                          lambda: f'Stel je maar weer voor dat je op de racebaan staat met jouw {self.user_model['kleur']} raceauto!',
                                          sleep_time=3)
        ...
        intervention_prep_choice.add_move('raceauto', self.droomrobot.say,
                                          'De auto heeft speciale zachte banden die over de speciale weg heen rijden.')
        intervention_prep_choice.add_move('raceauto', self.droomrobot.say,
                                          'Kijk maar wat voor mooie kleur de speciale weg heeft.')
        intervention_prep_choice.add_move('raceauto', self.droomrobot.say,
                                          'Je kunt je zelfs voorstellen dat de weg glitters heeft om je extra kracht te geven.')
        ...
        # Dolfijn
        intervention_prep_choice.add_move('dolfijn', self.droomrobot.say,
                                          lambda: f'Stel je je maar weer voor dat je een {self.user_model['kleur']} dolfijn in de zee bent.')
        intervention_prep_choice.add_move('dolfijn', self.droomrobot.say,
                                          'Kijk maar weer om je heen wat je allemaal ziet.')
        ...
        intervention_prep_choice.add_move('dolfijn', self.droomrobot.say,
                                          'Als je goed kijkt zie je in de verte een oude schatkist op de oceaanbodem.')
        intervention_prep_choice.add_move('dolfijn', self.droomrobot.say,
                                          'Dat is jouw missie van vandaag: je gaat ernaartoe zwemmen als een echte dolfijn.')
        intervention_prep_choice.add_move('dolfijn', self.droomrobot.say,
                                          'Je beweegt je lichaam soepel, net als een dolfijn.')
        ...
        intervention_prep_choice.add_move('dolfijn', self.droomrobot.say,
                                          'Je voelt hoe het water je zachtjes verder en lager brengt.')
        intervention_prep_choice.add_move('dolfijn', self.droomrobot.say,
                                          'Elke keer dat je inademt, voel je de frisse zeelucht vullen met energie.')
        intervention_prep_choice.add_move('dolfijn', self.droomrobot.say, 'Als je uitademt, laat je alle spanning los.')
        intervention_prep_choice.add_move('dolfijn', self.droomrobot.say,
                                          'Heel ontspannen, net als een dolfijn die rustig door het water glijdt.')
    ...
    def _build_interaction_choice_oefenen(self) -> InteractionChoice:
        ...
        interaction_choice.add_move('raceauto', self.droomrobot.say,
                                    'En op die plek staat een mooie raceauto, speciaal voor jou!')
        interaction_choice.add_move('raceauto', self.droomrobot.ask_entity_llm,
                                    'Kijk maar goed, welke kleur heeft jouw raceauto?', strict=True,
                                    user_model_key='kleur')
        interaction_choice.add_move('raceauto', self.droomrobot.get_adjective, lambda: self.user_model['kleur'],
                                    user_model_key='kleur_adjective')
        interaction_choice.add_move('raceauto', self.droomrobot.say,
                                    lambda: f"Wat goed, die mooie {self.user_model['kleur_adjective']} raceauto gaat jou vandaag helpen.")
        interaction_choice.add_move('raceauto', self.droomrobot.say,
                                    'Ga maar in de raceauto zitten, en voel maar hoe fijn je je daar voelt.',
                                    sleep_time=1)
        ...
        interaction_choice.add_move('raceauto', self.droomrobot.say,
                                    'Ga maar lekker een rondje rijden in je speciale auto en voel maar hoe makkelijk dat gaat.')
        ...
        interaction_choice.add_move('waterglijbaan', self.droomrobot.say,
                                    'Het is een lekker warme dag en je voelt de zon op je gezicht.')
        interaction_choice.add_move('waterglijbaan', self.droomrobot.say,
                                    'Om je heen hoor je het spetterende water en vrolijke stemmen.')
        interaction_choice.add_move('waterglijbaan', self.droomrobot.say,
                                    'En voor je zie je de grootste, gaafste waterglijbanen die je ooit hebt gezien!')
        interaction_choice.add_move('waterglijbaan', self.droomrobot.say,
                                    'Kijk maar eens om je heen, welke glijbanen je allemaal ziet, en welke kleuren de glijbanen hebben.')
        interaction_choice.add_move('waterglijbaan', self.droomrobot.ask_entity_llm,
                                    'Welke kleur heeft de glijbaan waar jij vanaf wilt gaan?', strict=True,
                                    user_model_key='kleur')
        interaction_choice.add_move('waterglijbaan', self.droomrobot.get_adjective, lambda: self.user_model['kleur'],
                                    user_model_key='kleur_adjective')
        interaction_choice.add_move('waterglijbaan', self.droomrobot.say,
                                    lambda: f'Wat goed! Die mooie {self.user_model['kleur_adjective']} glijbaan gaat jou vandaag helpen.')
        interaction_choice.add_move('waterglijbaan', self.droomrobot.say,
                                    'Je loopt langzaam de trap op naar de top van de glijbaan')
        interaction_choice.add_move('waterglijbaan', self.droomrobot.say,
                                    'Merk maar dat je je bij iedere stap fijner en rustiger voelt.')
        interaction_choice.add_move('waterglijbaan', self.droomrobot.say,
                                    'Misschien voel je een beetje kriebels in je buik dat is helemaal oké!')
        ...
        interaction_choice.add_move('waterglijbaan', self.droomrobot.say,
                                    'Ga maar rustig naar beneden met de glijbaan en merk maar hoe rustig en soepel dat gaat.')
        ...
        interaction_choice.add_move('dolfijn', self.droomrobot.say,
                                    'Kijk maar eens om je heen wat je allemaal kan zien.')
        interaction_choice.add_move('dolfijn', self.droomrobot.say,
                                    'Welke kleuren er allemaal zijn en hoe het er voelt.', sleep_time=2)
        interaction_choice.add_move('dolfijn', self.droomrobot.say,
                                    'Misschien is het water warm en zacht, of fris en koel.')
        interaction_choice.add_move('dolfijn', self.droomrobot.say, 'Je voelt hoe je licht wordt, alsof je zweeft.')
        interaction_choice.add_move('dolfijn', self.droomrobot.ask_entity_llm, 'Welke kleur dolfijn ben jij?',
                                    strict=True, user_model_key='kleur')
        interaction_choice.add_move('dolfijn', self.droomrobot.get_adjective, lambda: self.user_model['kleur'],
                                    user_model_key='kleur_adjective')
        interaction_choice.add_move('dolfijn', self.droomrobot.say,
        ...
        interaction_choice.add_move('dolfijn', self.droomrobot.say,
                                    'Ga maar lekker op avontuur in die onderwaterwereld en kijk wat je allemaal kan vinden.',
                                    sleep_time=2)
