...
class Bloedafname4(DroomrobotScript):
    ...
    def _introduction(self):
        ...
        self.add_move(self.droomrobot.ask_fake, 'Hoe heet jij?', 3, animated=True)
        self.add_move(self.droomrobot.say, f'{self.user_model['child_name']}, wat leuk je te ontmoeten.', animated=True)
        self.add_move(self.droomrobot.ask_fake, 'En hoe oud ben je?', 3, animated=True)
        self.add_move(self.droomrobot.say, f'{str(self.user_model['child_age'])} jaar. '
                                           f'Oh wat goed, dan ben je al oud genoeg om mijn speciale trucje te leren.', animated=True)
        self.add_move(self.droomrobot.say, 'Het is een truukje dat kinderen helpt om zich fijn en'
                                           'sterk te voelen in het ziekenhuis.', animated=True)
        ...
        self.add_move(self.droomrobot.say, 'Ik kan jou meenemen op een droomreis!', animated=True)
        ...
        self.add_move(self.droomrobot.say, 'Nu mag jij kiezen waar je heen wil in jouw droomreis.'
                                           'Je kunt kiezen uit het strand, het bos, de speeltuin of de ruimte.', animated=True)
        ...
        # # SAMEN OEFENEN
        self.add_move(self.droomrobot.say, 'Oke, laten we samen gaan oefenen.', animated=True)
        ...
        self.add_move(self.droomrobot.say, 'en blaas langzaam uit door je mond.', speaking_rate=0.75,
                      sleep_time=0.7)
        ...
        self.add_move(self.droomrobot.say, 'Hartstikke goed, ik ben benieuwd hoe goed het'
                                           ' lichtje je zometeen gaat helpen.', speaking_rate=0.75, sleep_time=0.7)
        self.add_move(self.droomrobot.say, 'Als je genoeg geoefend hebt, mag je je ogen weer '
                      ...
                      speaking_rate=0.75, sleep_time=3.0)
    ...
    def _intervention_preparation(self, phase_moves: InteractionChoice) -> InteractionChoice:
        ...
        phase_moves.add_move(InterventionPhase.PREPARATION.name, self.droomrobot.say, lambda: f'Wat fijn dat ik je weer mag helpen, we gaan weer samen een droomreis naar {self.user_model['droomplek_lidwoord']} {self.user_model['droomplek']} maken.',)
        ...
        phase_moves.add_move(InterventionPhase.PREPARATION.name, self.droomrobot.say,'Je mag weer goed gaan zitten, en je ogen dicht doen, zodat deze droomreis nog beter voor jou werkt.', sleep_time=1.0)
        ...
        phase_moves.add_move(InterventionPhase.PREPARATION.name, self.droomrobot.say, lambda:f'Stel je maar voor dat je bij {self.user_model['droomplek_lidwoord']} {self.user_model['droomplek']} bent.', speaking_rate=0.75, sleep_time=0.7)
        phase_moves.add_move(InterventionPhase.PREPARATION.name, self.droomrobot.say, 'Kijk maar weer naar alle mooie kleuren die om je heen zijn, en voel hoe fijn het is om daar te zijn.', speaking_rate=0.75, sleep_time=0.7)
    ...
    def _intervention_procedure(self, phase_moves: InteractionChoice) -> InteractionChoice:
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say, 'Nu gaan we je superkracht weer aanzetten, net zoals je hebt geleerd.', speaking_rate=0.75, sleep_time=0.7)
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say, 'Adem in door je neus.', speaking_rate=0.75, sleep_time=0.7)
        ...
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say, lambda: f'Je {self.user_model['kleur_adjective']} lichtje verschijnt weer op je arm.', speaking_rate=0.75, sleep_time=0.7)
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say, 'Zie het lichtje steeds sterker worden.', speaking_rate=0.75, sleep_time=0.7)
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say, 'Zo word jij weer een superheld, en kun je jezelf helpen.', speaking_rate=0.75, sleep_time=0.7)
        ...
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say, 'Dat betekent dat jouw kracht helemaal opgeladen is.', speaking_rate=0.75, sleep_time=0.7)
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say, 'je kunt het lichtje nog sterker maken door met je tenen te wiebelen.', speaking_rate=0.75, sleep_time=0.7)
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say, 'Het geeft een zachte, veilige gloed om je te helpen.', speaking_rate=0.75, sleep_time=0.7)
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say, 'Als je iets voelt op je arm, dan werkt de superkracht helemaal.', speaking_rate=0.75, sleep_time=0.7)
        ...
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.play_audio,'resources/audio/breath_out.wav')
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say,'Merk maar hoe goed jij jezelf kunt helpen, je bent echt een superheld.', speaking_rate=0.75, sleep_time=0.7)
        phase_moves.add_move(InterventionPhase.PROCEDURE.name, self.droomrobot.say, lambda: f'En nu je {self.user_model['kleur_adjective']} lichtje goed aan staat, kan jij weer verder spelen bij {self.user_model['droomplek_lidwoord']} {self.user_model['droomplek']}.', speaking_rate=0.75, sleep_time=0.7)
        sentences = [
    ...
    def _goodbye(self):
        self.add_move(self.droomrobot.say, lambda: f'Zo {self.user_model['child_name']}, het is weer tijd om doei te zeggen.', animated=True)
        self.add_move(self.droomrobot.say,'Wat fijn dat ik jou vandaag mocht helpen.', animated=True)
        ...
        ervaring_choice.add_move('positive', self.droomrobot.say, lambda: f'Wat fijn! je hebt jezelf echt goed geholpen, {self.user_model['child_name']}', animated=True)
        ...
        self.add_move(self.droomrobot.say, lambda: f'je kon al goed een {self.user_model['kleur']} lichtje uitzoeken.', animated=True)
        self.add_move(self.droomrobot.say, 'En weet je wat nu zo fijn is, hoe vaker je dit truukje oefent, hoe makkelijker het wordt.', animated=True)
        self.add_move(self.droomrobot.say, 'Je kunt dit ook zonder mij oefenen.', animated=True)
        self.add_move(self.droomrobot.say, lambda: f'Je hoeft alleen maar je ogen dicht te doen en aan je {self.user_model['kleur_adjective']} lichtje te denken.', animated=True)
        self.add_move(self.droomrobot.say, 'Dan word jij weer een superheld met extra kracht.', animated=True)
    ...
    def _build_interaction_choice_droomplek(self) -> InteractionChoice:
        ...
        interaction_choice.add_move('speeltuin', self.droomrobot.say,
                                    'De speeltuin, wat een vrolijke plek! Ik hou van de glijbaan en de schommel.', animated=True)
        interaction_choice.add_move('speeltuin', self.droomrobot.say,
                                    'Weet je wat ik daar graag doe? Heel hoog schommelen, bijna tot aan de sterren.', animated=True)
        ...
        interaction_choice.add_move('ruimte', self.droomrobot.say,
                                    'De ruimte, wat een avontuurlijke plek! Ik stel me voor dat ik in een raket zit en langs de sterren vlieg.', animated=True)
        interaction_choice.add_move('ruimte', self.droomrobot.say,
                                    'Weet je wat ik daar graag zou doen? Zwaaien naar de planeten en zoeken naar aliens die willen spelen.', animated=True)
